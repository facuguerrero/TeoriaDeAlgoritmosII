...
pattern = ""
DNA = ["DNA", 3, DNA_PATTERNS, DNA_FILES]
...
PI_PATTERNS = []
pi_alphabet = [str(i) for i in range(10)]
...
RANDOM_PATTERNS = []
random_alphabet = [i for i in range(65, 91)]
random_alphabet += [i for i in range(97, 123)]
...
CODE_PATTERNS = [['ifaaaa', 'define', 'ifndef', 'includ']]
...
def text_testing(algorithm, name, size, patterns, files, writer):
    ...
    print "\n  Performance test on ",name
    sys.stdout.write(RESET)
    sys.stdout.write(BOLD)
    print " files. Alphabet size: ",str(size)
    sys.stdout.write(RESET)
    #Main body
    for f in files:
        try:
        ...
        len_word = 1000
        i = 0
        while ((i + len_word + 1) < len(text)):
            new_pat.append(text[i:i+len_word])
            i+=len_word+1
        ...
        for pattern in patterns:
            #Setting P
            algorithm.set_n_patterns(pattern)
            #Printing and format
            print "    Searching for matchs with pattern len: ",str(len(pattern)),"."
            #Matching
            start = time.time()
            matches = algorithm.find_multiple_match()
            end = time.time()
            #Printing and format
            print "    Found ",str(len(matches))," matches on: "
            sys.stdout.write(YELLOW)
            print"         ", str(end - start) + "s."
            sys.stdout.write(RESET)
            if (str(algorithm) == "Rabin-Karp Algorithm"):
                writer.writerow([str(algorithm), algorithm.getHasher(), name, str(size), f[1], str(len(pattern)), end-start])
            else:
                writer.writerow([str(algorithm), "None", name, str(size), f[1], str(len(pattern)), end-start])
...
def main():
    ...
    import csv
    try:
        fd = open('results_rabinkarp.csv', 'w')
    except IOError:
        print "Error while trying to open file 'results.csv'."
        return
    writer = csv.writer(fd, delimiter = ",")
    writer.writerow(['Algorithm', 'Hash', 'File Type', 'Alphabet Size', 'Text Size', 'Pattern Size', 'Time'])
    #setup
    algorithms = [rabinKarp,naive]#, mp, kmp, colussi]#, rabinKarp, zt]
    #tests
    for algorithm in algorithms:
        sys.stdout.write(BOLD)
        #print ("\nBasic tests for: ", str(algorithm))
        lenHashers = 1
        if (str(algorithm) == "Rabin-Karp Algorithm"):
            lenHashers = len(hashers)

        for i in range(lenHashers):
            #si es el algoritmo de Rabin Karp se van seteando disntintos algoritmos
            if (str(algorithm) == "Rabin-Karp Algorithm"):
                algorithm.setHasher(hashers[i][1], hashers[i][0])
                #print ("  Hashing function: ", hashers[i][0])
            #basic tests
        ...
        for i in range(lenHashers):
            ...
            if (i == 0):
                ...
                print "Performance tests for: ",str(algorithm)
            if (str(algorithm) == "Rabin-Karp Algorithm"):
                algorithm.setHasher(hashers[i][1], hashers[i][0])
                sys.stdout.write(RED)
                print "\n  Hashing function: ",hashers[i][0]
            #Testing
            for test in TYPES:
