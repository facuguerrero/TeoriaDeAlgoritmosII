...
class KnuthMorrisPratt(StringMatching):
    """Implementación del algoritmo de Knuth Morris Pratt, segunda optimizacion
    del ingenuo, extension de MorrisPratt."""
    def __init__(self):
    ...
    def find_match(self):
        ...
        kmpNext = self.preprocessing(P, m)
        #Busqueda de matches
        i, j = 0, 0
        matchs = []
        #main loop
    ...
    def preprocessing(self, pattern, size):
        """Preprocesamiento del patrón.
            ...
            @return {List}: 'kmpNext', resultado del preprocesamiento de KMP.
        """
        #Se debe agregar un size+1 caracter por el ultimo if!!!
        pattern += " "
        #definicion de kmpNext
        kmpNext = [0 for i in xrange(size + 1)]
        kmpNext[0] = -1
        #inicializacion de variables
        i, j = 0, -1
        #Ciclo principal
        while (i < size):
            #La primera parte es igual a MorrisPratt
            while (j > -1) and (pattern[i] != pattern[j]):
                j = kmpNext[j]
            #avanzamos en i y en j
            i, j = i+1, j+1
            #Actualizamos la longitud de i
            #si es la misma letra, entonces tiene el mismo kmpNext
